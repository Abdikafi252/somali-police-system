...
@section('title', 'Wax ka bedel Saldhigga')

@section('content')
<div class="header" style="margin-bottom: 2rem; display: flex; justify-content: space-between; align-items: start;">
    <div>
        <h1 style="font-weight: 800; color: var(--sidebar-bg); font-family: 'Outfit';">WAX KA BEDEL SALDHIGGA</h1>
        <p style="color: var(--text-sub);">Cusboonaysii xogta saldhigga iyo maamulkiisa.</p>
    </div>
...
<div class="glass-card" style="max-width: 800px; padding: 2.5rem; border-radius: 25px;">
    <form action="{{ route('stations.update', $station->id) }}" method="POST">
        ...
        <div style="margin-bottom: 2rem;">
            ...
            <input type="text" name="station_name" id="station_name" value="{{ old('station_name', $station->station_name) }}" placeholder="Tusaale: Saldhigga Galbeed" 
        ...
        <div style="margin-bottom: 2.5rem;">
            ...
            <input type="text" name="location" id="location" value="{{ old('location', $station->location) }}" placeholder="Tusaale: Degmada Hodan, Muqdisho" 
                style="width: 100%; padding: 1.2rem; border-radius: 15px; border: 2px solid var(--border-soft); font-family: 'Inter', sans-serif; font-size: 1rem; font-weight: 600; background: #f8f9fa; color: #2d3436; transition: 0.3s;" required>
        </div>
